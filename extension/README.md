...
The extension guides the repeatability portion of the reliability gates without collecting private card details. On a Collectr portfolio, select **Start 2-capture comparison** and submit the first complete capture. Without changing the portfolio, start and submit a second capture. The popup stores only each capture's canonical fingerprint, entry and quantity counts, completeness, pagination result, and invalid-record count in local extension storage. It reports whether both fingerprints match and enables **Copy evidence summary** when at least one capture has completed.

Run one comparison for each of three distinct, user-controlled portfolios, including the free-account path and one portfolio with at least 1,000 holdings. The comparison intentionally does not log in, switch portfolios, manipulate browser accounts, or treat a partial capture as evidence. Start a new comparison whenever you switch portfolios.

The comparison is only one promotion gate. For two Pro portfolios, also produce a fresh CSV export without editing the portfolio between export and browser capture, and compare the canonical identity and quantity results. Finally, use **Dex write-contract research** to observe one manually performed, reversible decrease and one removal on a disposable test card. Copy the privacy-safe summaries and record the CSV comparison outcomes before asking to promote destructive-sync policy.
