...
To validate browser repeatability, open a Collectr portfolio in the extension and select **Start 5-capture series and capture run 1**. CardRelay captures, submits, and reloads the unchanged portfolio five times, stopping if a capture is incomplete or rejected. It automatically compares the canonical fingerprints and reports whether every capture is complete with zero invalid records. **Copy evidence summary** exports only fingerprints, counts, and pass/fail diagnostics—not card details or browser credentials. Repeat this for each required user-controlled portfolio. This helper does not authorize destructive writes; follow [the reliability gates](docs/browser-source-reliability.md) for the required CSV-equivalence, Dex decrease/removal contract, and operational-review evidence.
