...
The browser extension presents a card-level Collectr-to-Dex diff before write approval is available. General destructive Dex writes remain disabled until user-controlled observations establish validated decrease and removal contracts. Preview-confirmed additions and increases are enabled; a separately flagged single-managed-card removal test exists only to gather the removal-contract evidence. Read-after-write verification and per-operation audit results are required before those capabilities can be enabled.
