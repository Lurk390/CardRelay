...
2. **Repeatability:** each qualifying portfolio produces the same canonical collection fingerprint in two consecutive complete captures without account edits.
3. **CSV equivalence:** each qualifying Collectr Pro portfolio has one fresh export matching its browser capture at canonical identity, quantity, condition, finish, language, edition, and grading dimensions. Documented source limitations must be zero or explicitly approved.
